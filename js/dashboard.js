...
transactionForm.addEventListener('submit', async (e) => {
    ...
    const { data, error } = await supabase
        ...
        .select(); // Хадгалагдсан хариу болгож буцааж авах

    if (error) {
        alert("Гүйлгээг хадгалахад алдаа гарлаа: " + error.message);
    } else {
        alert("Гүйлгээ амжилттай нэмэгдлээ!");
        transactionForm.reset(); // Formiin buh talbariig tseverlej hoosn bolgono
        fetchTransactions(); 
    }
});
...
function renderTransactions(transactions) {
    ...
    //husnegtiig tseverleed, datag mur muruur ni zalgah
    let htmlContent = '';
